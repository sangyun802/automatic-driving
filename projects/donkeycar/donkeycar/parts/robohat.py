...
class RoboHATDriver:
    ...
    def set_pulse(self, steering, throttle):
        try:
            ...
            if throttle > 0:
                if steering>0.8:
                    output_throttle = dk.utils.map_range(throttle+0.02,
                                                     0, 1.0,
                                                     self.STOPPED_PWM, self.MAX_FORWARD)
                    #print("output_throttle : ", output_throttle)
                elif 0.1<=steering<=0.8:
                    output_throttle = dk.utils.map_range(throttle+0.01,
                                                     0, 1.0,
                                                     self.STOPPED_PWM, self.MAX_FORWARD)
                    #print("output_throttle : ", output_throttle)
                else:
