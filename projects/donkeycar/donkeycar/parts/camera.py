    ...
    def update(self):
        ...
        while self.on:
            ...
            if self.cam.query_image():
                ...
                '''cvt_frame=cv2.cvtColor(self.frame, cv2.COLOR_RGB2HSV)
                #lower_white=np.array([0,0,240])
                #upper_white=np.array([179, 15, 255])
                #lower_yellow=np.array([15,0,0])
                #upper_yellow=np.array([45, 255, 255])
                
                mask_white = cv2.inRange(cvt_frame, lower_white, upper_white)
                mask_yellow = cv2.inRange(cvt_frame, lower_yellow, upper_yellow)
                mask_or = cv2.bitwise_or(mask_white, mask_yellow)
                
                final_1= cv2.bitwise_and(mask_or, self.frame[:,:,0])
                final_2= cv2.bitwise_and(mask_or, self.frame[:,:,1])
                final_3= cv2.bitwise_and(mask_or, self.frame[:,:,2])
                #mask_canny = cv2.Canny(mask_or, 200, 400) 
                (width, height) = self.resolution
                #gray = cv2.cvtColor(self.frame,cv2.COLOR_RGB2GRAY)
                mask = np.zeros_like(final_1)
                polygon = np.array([[(0, height*1/2), (width, height*1/2), (width, height), (0, height)]], np.int32)
                cv2.fillPoly(mask, polygon, 255)
                mask_end_1 = cv2.bitwise_and(final_1, mask)
                mask_end_2 = cv2.bitwise_and(final_2, mask)
                mask_end_3 = cv2.bitwise_and(final_3, mask)

                self.frame[:,:,0] = mask_end_1
                self.frame[:,:,1] = mask_end_2
                self.frame[:,:,2] = mask_end_3'''
                ...
                polygon = np.array([[(0, height*1/2), (width, height*1/2), (width, height), (0, height)]], np.int32)
                cv2.fillPoly(mask, polygon, 255)
                self.frame[:,:,0] = cv2.bitwise_and(self.frame[:,:,0], mask)
                self.frame[:,:,1] = cv2.bitwise_and(self.frame[:,:,1], mask)
                self.frame[:,:,2] = cv2.bitwise_and(self.frame[:,:,2], mask)
                '''
